...
// AES256 복호화
export const aesDecrypt = function (encryptedString: string, key: string): string {
  const iv = Buffer.alloc(16, 0);
...
export const aesEncryptWithCryptoJSWithPadding = function (
  ...
  const cipher = CryptoJS.AES.encrypt(string, CryptoJS.enc.Utf8.parse(option.key), {
...
export const sha256EncryptWithSecretKey = function (string: string, secretKey: string) {
...
export { encryptValueWithCryptoOption, decryptValueWithCryptoOption, checkEncryptedString };
